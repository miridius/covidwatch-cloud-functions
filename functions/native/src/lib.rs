...
fn tcn_example(mut cx: FunctionContext) -> JsResult<JsString> {
    ...
    Ok(cx.string(format!("memo: {}", memo_data)))
}

register_module!(mut cx, { cx.export_function("tcn_example", tcn_example) });
